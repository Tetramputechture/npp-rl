...
class MockEnvironment:
    ...
    def _is_level_complete(self):
        ...
        exit_pos = (500, 300)
        distance_to_exit = np.sqrt(
            (self.ninja_pos[0] - exit_pos[0]) ** 2
...
class HierarchicalRLTrainer:
    ...
    def __init__(self, config: HierarchicalTrainingConfig):
        ...
        print(
            "  (In production, use real AdaptiveExplorationManager with proper level data)"
        )
    ...
    def _create_mock_hierarchical_manager(self):
        ...
        class MockHierarchicalManager:
            def get_available_subgoals(
                self, ninja_pos, level_data, switch_states, max_subgoals=8
            ):
                # Return mock subgoals
                return [
                    NavigationSubgoal(0.9, 20.0, 0.8, (500, 300), "exit_door", 400.0),
                    SwitchActivationSubgoal(
                        0.8, 30.0, 0.9, "switch_1", (200, 200), "door_switch", 0.85
                    ),
                ]
            ...
            while not done and total_steps < self.config.total_timesteps:
                ...
                if total_steps % self.config.log_frequency == 0:
                    self._log_training_progress(
                        episode_count, total_steps, episode_rewards
                    )
    ...
    def _update_subgoals(self, info: Optional[Dict] = None):
        ...
        if len(self.current_subgoals) > 0:
            ...
            for i, subgoal in enumerate(self.current_subgoals[:3]):  # Show top 3
                print(
                    f"   {i + 1}. {type(subgoal).__name__} (priority: {subgoal.priority:.2f})"
                )

    def _apply_reward_shaping(
        self, base_reward: float, ninja_pos, level_data, switch_states
    ) -> float:
        ...
        for subgoal in self.current_subgoals:
            ...
            weighted_reward = (
                shaping_reward * subgoal.priority * self.config.reward_shaping_weight
            )
            ...
            if subgoal.is_completed(ninja_pos, level_data, switch_states):
                ...
                print(
                    f"🏆 Subgoal completed: {type(subgoal).__name__} (+{completion_bonus:.1f} reward)"
                )
    ...
    def _log_training_progress(
        self, episode_count: int, total_steps: int, episode_rewards: List[float]
    ):
        ...
        print(
            f"  Subgoal generation time: {hierarchical_stats.get('planning_time_ms', 0):.2f}ms"
        )
        print(f"  Cache hit rate: {hierarchical_stats.get('cache_hit_rate', 0):.2%}")
        print(
            f"  Average subgoals: {hierarchical_stats.get('avg_subgoal_count', 0):.1f}"
        )
    ...
    def _display_performance_summary(self):
        ...
        if self.performance_metrics["subgoal_generation_time"]:
            ...
            p95_time = np.percentile(
                self.performance_metrics["subgoal_generation_time"], 95
            )
            ...
            print(
                f"  Target met: {'✅' if avg_time < self.config.performance_target_ms else '❌'}"
            )
        ...
        if self.performance_metrics["reward_shaping_bonus"]:
            ...
            print(
                f"  Total shaping contribution: {sum(self.performance_metrics['reward_shaping_bonus']):.1f}"
            )
        ...
        if self.performance_metrics["level_completion_rate"]:
            final_completion_rate = self.performance_metrics["level_completion_rate"][
                -1
            ]
            print(f"\nLevel Completion:")
...
def demonstrate_subgoal_types():
    ...
    subgoals = [navigation_subgoal, switch_subgoal]
