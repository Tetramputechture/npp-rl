...
class ExplorationMetrics:
    ...
    def end_episode(self, success: bool = False) -> Dict[str, float]:
        ...
        self.rolling_metrics['intrinsic_reward'].append(metrics['mean_intrinsic_reward'])
    ...
    def _compute_episode_metrics(self) -> Dict[str, float]:
        ...
        # Coverage metrics
        total_traversable_cells = self.grid_width * self.grid_height  # Simplified
...
class LevelComplexityMetrics:
    ...
    def update_step(
        ...
        if level_type in self.level_metrics:
            self.level_metrics[level_type].update_step(
                position, intrinsic_reward, info
            )
    ...
    def get_tensorboard_scalars(self) -> Dict[str, float]:
        ...
        for level_type, metrics in self.level_metrics.items():
            ...
            if self.level_counts[level_type] > 0:
                success_rate = self.level_successes[level_type] / self.level_counts[level_type]
                scalars[f'success_rate/{level_type}'] = success_rate
        
        return scalars


def create_exploration_callback(
