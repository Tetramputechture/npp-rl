...
class PhysicsStateExtractor:
    ...
    def extract_ninja_physics_state(
        ...
        potential_energy = np.clip(
            (NORMALIZED_HEIGHT_DIVISOR - y) / NORMALIZED_HEIGHT_DIVISOR, 0.0, 1.0
        )
        ...
        if isinstance(launch_pad_buffer, (list, tuple)):
            launch_pad_buffer = (
                launch_pad_buffer[0] if len(launch_pad_buffer) > 0 else 0
            )
        ...
        can_jump = (
            ...
            if (
                ground_contact > 0.5
                or jump_buffer > 0.0
                or (wall_contact > 0.5 and wall_buffer > 0.0)
            )
            else 0.0
        )
        ...
        launch_pad_proximity, hazard_proximity, collectible_proximity = (
            self._extract_entity_proximity(ninja_position, level_data)
        )

        # Extract advanced buffer states
        wall_slide_buffer, air_time, ground_time = self._extract_advanced_buffers(
            ninja_state
        )
        ...
        closest_exit_distance, switch_completion_ratio, path_efficiency = (
            self._get_multi_exit_features(ninja_position)
        )
        ...
        return np.array(
            [
                ...
                hazard_proximity,
                collectible_proximity,
                wall_slide_buffer,
                air_time,
                ground_time,
                max_jump_height,
                remaining_air_accel,
                closest_exit_distance,
                switch_completion_ratio,
                path_efficiency,
            ],
            dtype=np.float32,
        )
    ...
    def _estimate_wall_normal(
        self, ninja_position: Tuple[float, float], tiles: Any
    ) -> Tuple[float, float]:
    ...
    def _extract_entity_proximity(
        self, ninja_position: Tuple[float, float], level_data: Optional[Dict[str, Any]]
        ...
        for entity in entities:
            ...
            if dist < proximity_threshold:
                ...
                elif entity_type in [6, 7, 8]:  # Collectibles (switches, etc.)
    ...
    def _get_multi_exit_features(
        ...
        if closest_exit_distance == float("inf"):
            ...
            closest_exit_distance = max(
                0.0, 1.0 - (closest_exit_distance / level_diagonal)
            )
        ...
        if total_switches == 0:
            ...
            activated_switches = sum(
                1 for s in self._level_cache["switches"] if s["state"] > 0
            )
    ...
    def _extract_advanced_buffers(
        ...
        if isinstance(wall_slide_buffer, (list, tuple)):
            wall_slide_buffer = (
                wall_slide_buffer[0] if len(wall_slide_buffer) > 0 else 0
            )
    ...
    def _extract_physics_constraints(
        ...
        if vy < 0:  # Moving upward
            ...
            standard_jump_height = (JUMP_FLAT_GROUND_Y * JUMP_FLAT_GROUND_Y) / (
                2 * GRAVITY_JUMP
            )
