...
class HGTLayer(nn.Module):
    ...
    def forward(
        ...
        K = self._compute_type_specific_projections(
            node_features, node_types, self.k_linears
        )
        Q = self._compute_type_specific_projections(
            node_features, node_types, self.q_linears
        )
        V = self._compute_type_specific_projections(
            node_features, node_types, self.v_linears
        )
    ...
    def _compute_type_specific_projections(
        ...
        # Ensure node_types is long for indexing
    ...
    def _compute_attention_and_aggregate(
        ...
        # Ensure edge_types is long for indexing
