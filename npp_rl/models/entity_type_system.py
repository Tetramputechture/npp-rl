...
class EntitySpecializedEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.category_attention = nn.MultiheadAttention(
            embed_dim=output_dim, num_heads=8, dropout=dropout, batch_first=True
        )
    ...
    def forward(
        ...
        output = torch.zeros(
            batch_size, num_nodes, self.output_dim, device=node_features.device
        )
        ...
        for b in range(batch_size):
            for n in range(num_nodes):
                ...
                embedding = self.category_embeddings[category_name](
                    node_features[b : b + 1, n : n + 1]
                )
...
class HazardAwareAttention(nn.Module):
    ...
    def forward(
        ...
        if entity_types is not None:
            ...
            bias = (
                hazard_mask * self.hazard_bias + movement_mask * self.movement_bias
            ).unsqueeze(1)
