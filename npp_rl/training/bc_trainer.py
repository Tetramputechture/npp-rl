...
class BCTrainer:
    ...
    def __init__(
        ...
        # Create policy network with frame stacking configuration
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                if "player_frame" in observations:
                    ...
                    for i in range(min(len(frames), max_samples - samples_logged)):
                        ...
                        self.tensorboard_writer.add_image(
                            f"bc_samples/player_frame_{samples_logged}", img, epoch
                        )
                ...
                if "global_view" in observations:
                    ...
                    # Normalize to [0, 1] for visualization
                    views_norm = (global_views - global_views.min()) / (
                        global_views.max() - global_views.min() + 1e-8
                    )

                    for i in range(
                        min(len(global_views), max_samples - samples_logged)
                    ):
                        img = views_norm[i]
                        ...
                        self.tensorboard_writer.add_image(
                            f"bc_samples/global_view_{samples_logged}", img, epoch
                        )
    ...
    def _log_action_distributions(
        ...
        self.tensorboard_writer.add_histogram(
            f"bc_actions/{dataset_name}_expert", expert_actions, epoch
        )
        self.tensorboard_writer.add_histogram(
            f"bc_actions/{dataset_name}_predicted", predicted_actions, epoch
        )
        ...
        self.tensorboard_writer.add_image(
            f"bc_distributions/{dataset_name}_comparison", img_array, epoch
        )

        plt.close(fig)
        self.policy.train()

    def _log_observation_statistics(self, dataloader: DataLoader, epoch: int) -> None:
        ...
        for key, values in obs_dict.items():
            ...
            # Log histogram
            self.tensorboard_writer.add_histogram(
                f"bc_observations/{key}", values_flat, epoch
            )

            # Log statistics
            self.tensorboard_writer.add_scalar(
                f"bc_obs_stats/{key}_mean", values_flat.mean().item(), epoch
            )
            self.tensorboard_writer.add_scalar(
                f"bc_obs_stats/{key}_std", values_flat.std().item(), epoch
            )
    ...
    def train(
        ...
        if self.tensorboard_writer is not None:
            # Log dataset sizes
            self.tensorboard_writer.add_scalar(
                "bc_dataset/train_size", len(self.train_dataset), 0
            )
            self.tensorboard_writer.add_scalar(
                "bc_dataset/val_size", len(self.val_dataset), 0
            )

            # Log action distribution from the full dataset if available
            if hasattr(self.train_dataset, "dataset") and hasattr(
                self.train_dataset.dataset, "get_action_distribution"
            ):
                ...
                for action_idx, name in enumerate(action_names):
                    if action_idx in action_dist:
                        ...
                        self.tensorboard_writer.add_scalar(
                            f"bc_dataset/action_{name}_percentage", percentage, 0
                        )

            # Log normalization statistics if available
            if hasattr(self.train_dataset, "dataset") and hasattr(
                self.train_dataset.dataset, "normalizer"
            ):
                ...
                if normalizer.stats:
                    for key, stats in normalizer.stats.items():
                        ...
                        if isinstance(mean, np.ndarray):
                            ...
                            self.tensorboard_writer.add_scalar(
                                f"bc_normalization/{key}_std_avg", float(std.mean()), 0
                            )
        ...
        for epoch in range(1, epochs + 1):
            ...
            if self.tensorboard_writer is not None:
                ...
                if epoch == 1 or epoch % 5 == 0:
                    ...
                    self._log_action_distributions(
                        train_loader, epoch, dataset_name="train"
                    )
                    self._log_action_distributions(
                        val_loader, epoch, dataset_name="val"
                    )
