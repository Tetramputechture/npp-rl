"""
Consolidated Reachability-Aware Intrinsic Curiosity Module (ICM)

This module provides a clean, consolidated implementation that integrates with
nclone's existing reachability systems rather than duplicating functionality.
...
class ConsolidatedICMNetwork(nn.Module):
    """
    Consolidated ICM that integrates with nclone's reachability systems.
    ...
    def __init__(
        ...
        """
        Initialize consolidated reachability-aware ICM.
        ...
        Args:
            ...
            enable_reachability_awareness: Enable reachability modulation
        ...
        if enable_reachability_awareness and NCLONE_AVAILABLE:
            self.reachability_calculator = ReachabilityAwareExplorationCalculator(debug=debug)
        else:
            self.reachability_calculator = None
            if enable_reachability_awareness and not NCLONE_AVAILABLE:
                print("Warning: Reachability awareness requested but nclone not available")
        
        # Performance tracking
        self.computation_times = deque(maxlen=100)
    
    def forward(self, features_current: torch.Tensor, features_next: torch.Tensor, actions: torch.Tensor):
    ...
    def compute_intrinsic_reward(
        ...
        with torch.no_grad():
            _, predicted_next_features = self.forward(features_current, features_next, actions)
            prediction_error = F.mse_loss(predicted_next_features, features_next, reduction='none')
            base_curiosity = prediction_error.mean(dim=1) * self.eta
        
        # Apply reachability modulation if enabled and available
        # Skip if no level data to avoid expensive failed computations
        if (self.enable_reachability_awareness and 
            self.reachability_calculator is not None and 
            observations is not None and
            observations.get("level_data") is not None):
    ...
    def _apply_reachability_modulation(
        ...
        for i in range(batch_size):
            ...
            reward_info = self.reachability_calculator.calculate_reachability_aware_reward(
    ...
    def _extract_single_observation(self, observations: Dict[str, Any], index: int) -> Dict[str, Any]:
...
class ConsolidatedICMTrainer:
    """
    Trainer for consolidated reachability-aware ICM.
    ...
    def __init__(
        self,
        icm_network: ConsolidatedICMNetwork,
    ...
    def update(
        ...
        # Compute losses
        losses = self.icm_network.compute_losses(features_current, features_next, actions)
...
ICMNetwork = ConsolidatedICMNetwork
ICMTrainer = ConsolidatedICMTrainer