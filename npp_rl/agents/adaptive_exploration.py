...
class CuriosityModule(nn.Module):
    ...
    def __init__(self, feature_dim: int, action_dim: int = 5, hidden_dim: int = 256):
        ...
        self.forward_model = nn.Sequential(
            nn.Linear(64 + action_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, 64)
        )

        # Inverse model: predicts action from current and next state features
        self.inverse_model = nn.Sequential(
            nn.Linear(64 * 2, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, action_dim)
        )
    ...
    def forward(self, state, action, next_state):
        ...
        intrinsic_reward = torch.norm(
            predicted_next_phi - phi_next_state.detach(), dim=1
        )
...
class NoveltyDetector:
    ...
    def get_novelty_bonus(self, state_features: torch.Tensor) -> float:
        ...
        state_hash = (
            hash(tuple(state_features.detach().cpu().numpy().round(2).tolist()))
            % self.hash_dim
        )
...
class AdaptiveExplorationManager:
    ...
    def __init__(
        ...
        self.curiosity_module = (
            None  # Will be initialized when we have feature dimensions
        )
    ...
    def get_exploration_bonus(self, state, action, next_state) -> float:
        ...
        if self.curiosity_module is not None:
            with torch.no_grad():
                _, _, curiosity_reward = self.curiosity_module(
        ...
        novelty_bonus = (
            self.novelty_detector.get_novelty_bonus(state) * self.novelty_weight
        )
    ...
    def get_available_subgoals(
        ...
        if cache_key in self.subgoal_cache:
            ...
            if time.time() - cached_result["timestamp"] < self.cache_ttl / 1000.0:
                self.cache_hit_rate = (
                    0.9 * self.cache_hit_rate + 0.1 * 1.0
                )  # Update hit rate
        ...
        self.avg_subgoal_count = 0.9 * self.avg_subgoal_count + 0.1 * len(
    ...
    def _generate_cache_key(
        self, ninja_pos: Tuple[float, float], switch_states: Dict, level_data=None
    ) -> str:
    ...
    def _cleanup_cache(self):
        ...
        expired_keys = [
            key
            for key, value in self.subgoal_cache.items()
    ...
    def update_subgoals_on_switch_change(
        self,
        ninja_pos: Tuple[float, float],
        level_data,
        old_switch_states: Dict,
        new_switch_states: Dict,
        ...
        old_subgoals = self.get_available_subgoals(
            ninja_pos, level_data, old_switch_states
        )

        # Get new subgoals with updated switch states
        new_subgoals = self.get_available_subgoals(
            ninja_pos, level_data, new_switch_states
        )
        ...
        newly_available = [
            s
            for s in new_subgoals
            if (s.position[0], s.position[1]) not in old_positions
        ]
    ...
    def get_statistics(self) -> Dict[str, float]:
        ...
        return {
            ...
            "avg_completion_time": np.mean(list(self.recent_completion_times))
