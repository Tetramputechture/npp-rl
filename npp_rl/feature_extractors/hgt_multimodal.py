...
class DummyReachabilityExtractor:
    """
    Dummy reachability extractor for graceful degradation when nclone is unavailable.
    
    This class provides a fallback implementation that returns zero features,
    allowing the model to function during development and testing even when
    the full nclone reachability system is not available.
    """
    
    def extract_features(self, ninja_pos, level_data, switch_states, performance_target="fast"):
...
class HGTMultimodalExtractor(BaseFeaturesExtractor):
    ...
    def __init__(
        ...
        enable_reachability_features: bool = True,
        # Fusion parameters
        ...
        **kwargs
        ...
        Args:
            ...
            enable_reachability_features: Whether to enable reachability integration
        ...
        if self.enable_reachability_features:
            ...
            # Calculate total feature dimension without reachability
        ...
        for i in range(batch_size):
            try:
                features = self.reachability_extractor.extract_features(
                    ninja_pos=ninja_positions[i] if ninja_positions is not None else (0.0, 0.0),
                ...
                features = torch.zeros(self.reachability_dim, dtype=torch.float32, device=device)
                reachability_features.append(features)
        
        return torch.stack(reachability_features).to(device)
    
    def _extract_ninja_positions(self, observations: Dict[str, torch.Tensor]) -> Optional[List[Tuple[float, float]]]:
    ...
    def _extract_level_data(self, observations: Dict[str, torch.Tensor]) -> Optional[List[Any]]:
        """Extract level data from observations."""
        # This is a placeholder - actual implementation would depend on observation format
        # For now, return None to trigger fallback behavior
        return None
    
    def _extract_switch_states(self, observations: Dict[str, torch.Tensor]) -> Optional[List[Dict[str, bool]]]:
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self.enable_reachability_features:
            ...
            if visual_features is not None and graph_features is not None and state_features is not None:
            ...
            # Standard processing without reachability features
            features = []
            if visual_features is not None:
