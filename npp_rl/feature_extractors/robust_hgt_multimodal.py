...
class TemporalCNN3D(nn.Module):
    ...
    def __init__(self, input_channels: int = 1, output_dim: int = 512):
        ...
        self.adaptive_pool = nn.AdaptiveAvgPool3d(POOLING_CONFIG.adaptive_pool3d_output_size)
        
        # Feature projection
        pooled_size = (POOLING_CONFIG.cnn_final_channels * 
...
class SpatialCNN2D(nn.Module):
    ...
    def __init__(self, input_channels: int = 1, output_dim: int = 256):
        ...
        # Adaptive pooling
        self.adaptive_pool = nn.AdaptiveAvgPool2d(POOLING_CONFIG.adaptive_pool2d_output_size)
    ...
    def forward(self, x: torch.Tensor, graph_features: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
...
class RobustHGTMultimodalExtractor(BaseFeaturesExtractor):
    """
    ...
    def __init__(
        ...
        self.temporal_cnn = TemporalCNN3D(
            input_channels=1,
            output_dim=DEFAULT_CONFIG.embed_dim
        )
        
        # 2. Spatial processing (2D CNN for global view)
        self.spatial_cnn = SpatialCNN2D(
            input_channels=1,
            output_dim=DEFAULT_CONFIG.global_hidden_dim
        )
        ...
        if self.debug:
            self.logger.info("Initialized RobustHGTMultimodalExtractor")
