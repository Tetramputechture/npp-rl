...
class DummyReachabilityExtractor:
    """
    Dummy reachability extractor for graceful degradation.
    
    Provides fallback behavior when nclone reachability system is unavailable,
    returning zero features to maintain compatibility during development.
    """
...
class ReachabilityWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        if self.debug:
            print(f"ReachabilityWrapper initialized with cache_ttl={cache_ttl_ms}ms, "
                  f"performance_target={performance_target}")
    
    def _initialize_reachability_extractor(self):
        """Initialize reachability feature extractor with graceful fallback."""
        try:
            ...
            if self.debug:
                print(f"nclone reachability system not available: {e}")
                print("Using dummy extractor for graceful degradation")
    ...
    def _extend_observation_space(self):
        ...
        if self.debug:
            print(f"Extended observation space with reachability features: {reachability_space}")
    ...
    def _generate_cache_key(self, ninja_pos: Tuple[float, float], switch_states: Dict[str, bool]) -> str:
    ...
    def _cache_features(self, cache_key: str, features: np.ndarray, extraction_time: float):
    ...
    def _clean_cache(self):
        ...
        if self.debug:
            print(f"Cleaned {entries_to_remove} cache entries, "
