...
class ConfigurableMultimodalExtractor(BaseFeaturesExtractor):
    ...
    def _create_temporal_cnn(self, visual_config) -> nn.Module:
        ...
        return nn.Sequential(
            ...
            nn.Conv3d(1, visual_config.temporal_channels[0], kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=1),
            nn.BatchNorm3d(visual_config.temporal_channels[0]),
            nn.ReLU(inplace=True),
            nn.Dropout3d(visual_config.cnn_dropout),
            
            nn.Conv3d(visual_config.temporal_channels[0], visual_config.temporal_channels[1], 
                     kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=1),
            nn.BatchNorm3d(visual_config.temporal_channels[1]),
            nn.ReLU(inplace=True),
            nn.Dropout3d(visual_config.cnn_dropout),
            
            nn.Conv3d(visual_config.temporal_channels[1], visual_config.temporal_channels[2],
                     kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=1),
            ...
            nn.Flatten(),
            nn.Linear(visual_config.temporal_channels[2] * 1 * 4 * 4, visual_config.temporal_output_dim),
            nn.ReLU(),
            nn.Dropout(0.2),
        )
    ...
    def _create_global_cnn(self, visual_config) -> nn.Module:
        ...
        return nn.Sequential(
            ...
            nn.Conv2d(1, visual_config.global_channels[0], kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(visual_config.global_channels[0]),
            nn.ReLU(inplace=True),
            nn.Dropout2d(visual_config.cnn_dropout),
            
            nn.Conv2d(visual_config.global_channels[0], visual_config.global_channels[1],
                     kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(visual_config.global_channels[1]),
            nn.ReLU(inplace=True),
            nn.Dropout2d(visual_config.cnn_dropout),
            
            nn.Conv2d(visual_config.global_channels[1], visual_config.global_channels[2],
                     kernel_size=3, stride=2, padding=1),
            ...
            nn.Flatten(),
            nn.Linear(visual_config.global_channels[2] * 4 * 4, visual_config.global_output_dim),
            nn.ReLU(),
            nn.Dropout(0.2),
        )
    ...
    def _create_state_mlp(self, input_dim: int, hidden_dim: int, output_dim: int) -> nn.Module:
    ...
    def _create_reachability_mlp(self, input_dim: int, hidden_dim: int, output_dim: int) -> nn.Module:
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self.graph_encoder is not None and "graph_obs" in observations:
            ...
            if isinstance(self.graph_encoder, SimplifiedHGTEncoder):
                node_types = graph_obs.get("node_types", torch.zeros(node_features.shape[:2], dtype=torch.long))
                _, graph_features = self.graph_encoder(node_features, edge_index, node_types, node_mask)
            else:
                _, graph_features = self.graph_encoder(node_features, edge_index, node_mask)
        ...
        if len(features) == 0:
            raise ValueError("No features were extracted! Check configuration and observation space.")
...
class SingleHeadFusion(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int, dropout: float = 0.1):
        super().__init__()
        self.attention = nn.MultiheadAttention(input_dim, num_heads=1, dropout=dropout, batch_first=True)
...
class MultiHeadFusion(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int, num_heads: int = 8, dropout: float = 0.1):
        super().__init__()
        self.attention = nn.MultiheadAttention(input_dim, num_heads=num_heads, dropout=dropout, batch_first=True)
        self.norm = nn.LayerNorm(input_dim)
        self.mlp = nn.Sequential(
            nn.Linear(input_dim, output_dim),
            nn.ReLU(),
