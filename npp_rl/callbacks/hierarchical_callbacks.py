...
class HierarchicalStabilityCallback(BaseCallback):
    ...
    def _log_stability_metrics(self):
        ...
        if self.instability_reasons:
            self.logger.record("stability/instability_count", len(self.instability_reasons))
    ...
    def _detect_instability(self):
        ...
        if not self.is_stable and self.verbose > 0:
            warnings.warn(f"Training instability detected: {'; '.join(self.instability_reasons)}")
...
class ExplorationMetricsCallback(BaseCallback):
    ...
    def _log_exploration_metrics(self):
        ...
        if self.intrinsic_rewards:
            self.logger.record("icm/intrinsic_reward_mean", np.mean(self.intrinsic_rewards))
            self.logger.record("icm/intrinsic_reward_std", np.std(self.intrinsic_rewards))
            self.logger.record("icm/intrinsic_reward_max", np.max(self.intrinsic_rewards))
        
        # ICM losses
        if self.icm_forward_losses:
            self.logger.record("icm/forward_loss_mean", np.mean(self.icm_forward_losses))
        
        if self.icm_inverse_losses:
            self.logger.record("icm/inverse_loss_mean", np.mean(self.icm_inverse_losses))
...
class CurriculumProgressionCallback(BaseCallback):
    ...
    def _evaluate_progression(self):
        ...
        if self.logger:
            ...
            self.logger.record("curriculum/difficulty", 
                             {'simple': 0, 'medium': 1, 'complex': 2}[self.current_difficulty])
        
        # Check progression
        if self.current_difficulty == 'simple' and success_rate >= self.simple_threshold:
            self.current_difficulty = 'medium'
            if self.verbose > 0:
                print(f"Progressing to medium difficulty (success rate: {success_rate:.2%})")
        
        elif self.current_difficulty == 'medium' and success_rate >= self.medium_threshold:
            self.current_difficulty = 'complex'
            if self.verbose > 0:
                print(f"Progressing to complex difficulty (success rate: {success_rate:.2%})")
