...
class IntrinsicRewardWrapper(gym.Wrapper):
    ...
    def step(self, action: Union[int, np.ndarray]) -> Tuple[Any, float, bool, bool, Dict[str, Any]]:
        """
    ...
    def _compute_intrinsic_reward(
        ...
        try:
            ...
            int_reward = clip_intrinsic_reward(
                int_rewards, clip_max=self.r_int_clip
            )[0]
            ...
            if (self.step_count % self.update_frequency == 0 and 
                len(self.experience_buffer) >= 32):
                self._update_icm()
            
            return float(int_reward)
            
        except Exception as e:
            # Fallback to zero intrinsic reward if computation fails
            if self.enable_logging:
                print(f"Warning: ICM computation failed: {e}")
            return 0.0
    ...
    def _log_episode_stats(self, info: Dict[str, Any]):
        ...
        self.global_stats['mean_r_ext'] = (
            (1 - alpha) * self.global_stats['mean_r_ext'] + 
            alpha * self.episode_stats['r_ext_sum']
        )
        self.global_stats['mean_r_int'] = (
            (1 - alpha) * self.global_stats['mean_r_int'] + 
