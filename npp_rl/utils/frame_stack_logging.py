        ...
        if player_frames.ndim == 5:
            ...
            for i in range(batch_size):
                ...
                vis_img = visualize_stacked_frames(
                    stacked_frame,
                    title=f"Player Frame Stack (Sample {i})"
                )
    ...
    if "global_view" in observations:
        ...
        if global_views.ndim == 5:
            ...
            for i in range(batch_size):
                ...
                vis_img = visualize_stacked_frames(
                    stacked_view,
                    title=f"Global View Stack (Sample {i})"
                )
...
def log_state_stack_statistics(
    ...
    if game_state.ndim == 3:
        ...
        for t in range(stack_size):
            ...
            writer.add_scalar(
                f"{prefix}_stack/mean_t_{t}",
                np.mean(state_t),
                global_step
            )
            writer.add_scalar(
                f"{prefix}_stack/std_t_{t}",
                np.std(state_t),
                global_step
            )
        ...
        if stack_size > 1:
            ...
            writer.add_scalar(
                f"{prefix}_stack/temporal_change_std",
                np.std(state_diff),
                global_step
            )
