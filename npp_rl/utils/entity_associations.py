...
class EntityAssociationManager:
    ...
    def extract_entity_pairs_from_sim(self, sim) -> LevelCompletionInfo:
        ...
        if hasattr(sim, 'entity_dic'):
            ...
            for locked_door in locked_doors:
                ...
                locked_door_pairs.append(EntityPair(
                    ...
                    switch_pos=(locked_door.xpos, locked_door.ypos),  # Switch position
            ...
            for trap_door in trap_doors:
                ...
                trap_door_pairs.append(EntityPair(
        ...
        accessible_exits = [pair for pair in exit_pairs if pair.switch_active and pair.door_accessible]
        required_switches = [pair for pair in exit_pairs if not (pair.switch_active and pair.door_accessible)]
    ...
    def find_best_exit_option(
        ...
        if completion_info.accessible_exits:
            best_exit = min(
                ...
                key=lambda pair: self._calculate_path_cost(start_pos, pair.door_pos, graph_data)
            )
            return best_exit
        ...
        if completion_info.required_switches:
            best_sequence = min(
                ...
                key=lambda pair: (
                    ...
                    self._calculate_path_cost(pair.switch_pos, pair.door_pos, graph_data)
                )
            )
    ...
    def _calculate_graph_path_cost(
        ...
        if hasattr(graph_data, 'resolution_info'):
            sub_cell_dims = graph_data.resolution_info['grid_dimensions'].get(0, (176, 100))  # 6px resolution
        ...
        # Bounds checking
        if (start_node >= graph_data.sub_cell_graph.num_nodes or 
            end_node >= graph_data.sub_cell_graph.num_nodes or
            start_node < 0 or end_node < 0):
        ...
        if hasattr(graph_data, 'region_graph'):
            ...
            region_distance = abs(end_region_col - start_region_col) + abs(end_region_row - start_region_row)
    ...
    def get_completion_requirements(self, completion_info: LevelCompletionInfo) -> List[Dict[str, Any]]:
        """
        ...
        if completion_info.completion_strategy == 'direct_exit':
            requirements.append({
                'type': 'reach_exit',
                'priority': 1,
                'positions': [pair.door_pos for pair in completion_info.accessible_exits],
            ...
            requirements.append({
                ...
                'priority': 2,
                'positions': [pair.switch_pos for pair in completion_info.required_switches],
            ...
            requirements.append({
                ...
                'priority': 1,
                'positions': [pair.door_pos for pair in completion_info.required_switches],
        ...
        if completion_info.locked_door_pairs:
            blocking_locked_doors = [pair for pair in completion_info.locked_door_pairs 
                                   if not pair.switch_active and not pair.door_accessible]
            if blocking_locked_doors:
                requirements.append({
                    'type': 'consider_locked_doors',
                    'priority': 3,
                    'positions': [pair.switch_pos for pair in blocking_locked_doors],
        ...
        if completion_info.trap_door_pairs:
            risky_traps = [pair for pair in completion_info.trap_door_pairs 
