...
class S3Uploader:
    ...
    def __init__(
        self, bucket: str, prefix: str, experiment_name: str, dry_run: bool = False
    ):
        ...
        if not dry_run:
            try:
                ...
                logger.info(
                    f"S3 uploader initialized: s3://{bucket}/{self.base_prefix}"
                )
    ...
    def upload_file(
        ...
        try:
            ...
            logger.info(
                f"Uploaded: {local_path.name} -> s3://{self.bucket}/{full_s3_key}"
            )
            return True
    ...
    def upload_directory(
        self, local_dir: str, s3_prefix: str, pattern: str = "*", recursive: bool = True
    ) -> int:
    ...
    def sync_tensorboard_logs(self, tensorboard_dir: str, s3_prefix: str) -> int:
        ...
        return self.upload_directory(
            tensorboard_dir, s3_prefix, pattern="events.out.tfevents.*", recursive=True
        )
...
def create_s3_uploader(
    bucket: Optional[str], prefix: str, experiment_name: str, dry_run: bool = False
) -> Optional[S3Uploader]:
