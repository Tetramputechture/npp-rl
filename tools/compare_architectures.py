"""
Architecture comparison tool for Task 3.1.
...
def create_observation_space() -> SpacesDict:
    ...
    return SpacesDict({
        ...
        "game_state": Box(low=-float('inf'), high=float('inf'), shape=(30,), dtype='float32'),
        "reachability_features": Box(low=-float('inf'), high=float('inf'), shape=(8,), dtype='float32'),
        "graph_obs": SpacesDict({
            "node_features": Box(low=-float('inf'), high=float('inf'), shape=(100, 67), dtype='float32'),
...
def build_and_benchmark_architecture(
    ...
    try:
        ...
        print(f"  - Modalities: {', '.join(config.modalities.get_enabled_modalities())}")
        ...
        print(f"  - Inference time: {results.mean_inference_time_ms:.2f} ± {results.std_inference_time_ms:.2f} ms")
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare different model architectures for NPP-RL (Task 3.1)"
    )
    parser.add_argument(
        '--architectures',
        nargs='+',
    ...
    parser.add_argument(
        '--all',
        action='store_true',
        help='Compare all available architectures'
    )
    parser.add_argument(
        '--list',
        action='store_true',
    ...
    print(f"\n{'='*60}")
    print(f"Architecture Comparison Tool (Task 3.1)")
    print(f"{'='*60}")
    ...
    sample_observations = create_mock_observations(batch_size=args.batch_size, device=device)
    ...
    if all_results:
        ...
        print("\nKey Findings:")
        print(f"  Fastest Architecture: {comparison['fastest']['name']} "
              f"({comparison['fastest']['time_ms']:.2f} ms)")
        print(f"  Most Memory Efficient: {comparison['most_memory_efficient']['name']} "
              f"({comparison['most_memory_efficient']['memory_mb']:.2f} MB)")
        print(f"  Smallest Model: {comparison['smallest']['name']} "
              f"({comparison['smallest']['num_params']:,} parameters)")
        
        # Recommendations based on Task 3.1 criteria
        print("\n" + "="*80)
        print("RECOMMENDATIONS (based on Task 3.1 criteria)")
        print("="*80)
        ...
        for i, r in enumerate(fast_archs, 1):
            speedup = all_results[0].mean_inference_time_ms / r.mean_inference_time_ms \
                     if r != all_results[0] else 1.0
            print(f"  {i}. {r.architecture_name}: {r.mean_inference_time_ms:.2f} ms "
        ...
        for i, r in enumerate(mem_archs, 1):
            print(f"  {i}. {r.architecture_name}: {r.parameter_memory_mb:.2f} MB "
                  f"({r.num_parameters:,} params)")
        
        print("\nNext Steps:")
        print("  1. Train selected architectures on actual N++ levels (once training set is ready)")
        print("  2. Evaluate final performance and convergence speed")
        print("  3. Select architecture based on weighted criteria (Task 3.1)")
