...
class ReplayValidator:
    ...
    def validate_frame(frame_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        required_fields = ['timestamp', 'level_id', 'frame_number', 'player_state', 
        ...
        # Validate timestamp
        if not isinstance(frame_data['timestamp'], (int, float)) or frame_data['timestamp'] <= 0:
            errors.append("Invalid timestamp")
        
        # Validate frame number
        if not isinstance(frame_data['frame_number'], int) or frame_data['frame_number'] < 0:
        ...
        if not all(isinstance(level_bounds.get(k), int) and level_bounds.get(k) > 0 
...
class ObservationProcessor:
    ...
    def _get_env(self) -> BasicLevelNoGold:
...
class ReplayIngester:
            ...
            with open(file_path, 'r') as f:
                for line_num, line in enumerate(f, 1):
                    try:
                        ...
                        if not is_valid:
                            logger.warning(f"{file_path}:{line_num} - Validation errors: {errors}")
    ...
    def print_statistics(self):
        ...
        if self.stats['frames_processed'] > 0:
            validity_rate = self.stats['frames_valid'] / self.stats['frames_processed'] * 100
            print(f"Validity rate: {validity_rate:.1f}%")


def create_sample_replay_data(output_path: Path, num_frames: int = 100):
    ...
    with open(output_path, 'w') as f:
        for i in range(num_frames):
            frame = {
                "timestamp": 1692345678.123 + i * 0.016,  # 60 FPS
                "level_id": "test_level_001",
                "frame_number": i,
                "player_state": {
                    "position": {"x": 150.5 + i * 0.5, "y": 200.3 - i * 0.1},
                    "velocity": {"x": 2.1, "y": -0.5},
                    "on_ground": i % 10 != 0,
                    "wall_sliding": False,
                    "jump_time_remaining": max(0, 1.0 - i * 0.1) if i < 10 else 0.0
                },
                "player_inputs": {
                    "left": False,
                    "right": i % 20 < 15,  # Right most of the time
                    "jump": i % 30 < 5,    # Jump occasionally
                    "restart": False
                },
                "entities": [
                    {
    ...
    logger.info(f"Created sample replay data: {output_path}")
...
Examples:
  ...
  python tools/replay_ingest.py --create-sample datasets/raw/sample.jsonl --num-frames 200
  ...
  python tools/replay_ingest.py --input datasets/raw --dry-run
    ...
    parser.add_argument('--input', type=Path, help='Input directory with JSONL replay files')
    parser.add_argument('--output', type=Path, help='Output directory for processed datasets')
    parser.add_argument('--profile', choices=['minimal', 'rich'], default='rich',
                       help='Observation profile to use')
    parser.add_argument('--format', choices=['npz', 'parquet'], default='npz',
                       help='Output format')
    parser.add_argument('--max-samples', type=int, default=10000,
                       help='Maximum samples per output file')
    parser.add_argument('--create-sample', type=Path,
                       help='Create sample replay data at specified path')
    parser.add_argument('--num-frames', type=int, default=100,
                       help='Number of frames in sample data')
    ...
    if args.create_sample:
        create_sample_replay_data(args.create_sample, args.num_frames)
        return
    ...
    ingester = ReplayIngester(
        observation_profile=args.profile,
