...
class DataValidator:
        ...
        for key, array in observations.items():
            if key == "player_frame":
                ...
                if array.shape[1:] != expected_shape:
                    errors.append(
                        f"Invalid {key} shape: {array.shape[1:]} != {expected_shape}"
                    )

                if array.dtype != self.EXPECTED_DTYPES["player_frame"]:
                    errors.append(
                        f"Invalid {key} dtype: {array.dtype} != {self.EXPECTED_DTYPES['player_frame']}"
                    )
                ...
                if array.dtype != self.EXPECTED_DTYPES["global_view"]:
                    errors.append(
                        f"Invalid {key} dtype: {array.dtype} != {self.EXPECTED_DTYPES['global_view']}"
                    )
                ...
                if array.shape[1] == 17:
                    ...
                    errors.append(
                        f"Invalid {key} shape: {array.shape[1]} not in [17, 31]"
                    )
                    continue
                ...
                if array.dtype != self.EXPECTED_DTYPES["game_state"]:
                    errors.append(
                        f"Invalid {key} dtype: {array.dtype} != {self.EXPECTED_DTYPES['game_state']}"
                    )
    ...
    def _validate_actions(self, actions: np.ndarray) -> Tuple[List[str], List[str]]:
        ...
        if actions.dtype != self.EXPECTED_DTYPES["actions"]:
            errors.append(
                f"Invalid actions dtype: {actions.dtype} != {self.EXPECTED_DTYPES['actions']}"
            )
        ...
        if max_freq > 0.8:
            warnings.append(
                f"Action distribution highly skewed: max frequency {max_freq:.2f}"
            )
    ...
    def _compute_quality_metrics(
        ...
        # Action distribution
        action_distribution = {
            i: int(count) for i, count in enumerate(np.bincount(actions, minlength=6))
        }
...
class TrajectoryDeduplicator:
    ...
    def compute_trajectory_hash(self, actions: np.ndarray, meta: Dict[str, Any]) -> str:
        ...
        signature_data = {
            ...
            "level_id": meta.get("level_id", ["unknown"])[0]
            if isinstance(meta.get("level_id"), np.ndarray)
            else meta.get("level_id", "unknown"),
            "length": len(actions),
    ...
    def compute_trajectory_similarity(
        self, actions1: np.ndarray, actions2: np.ndarray
    ) -> float:
        """
...
Examples:
  ...
  python tools/data_quality.py --validate datasets/processed/batch_0000.npz
    ...
    parser.add_argument(
        "--deduplicate", type=Path, help="Deduplicate datasets in directory"
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    if args.validate:
        ...
        if not dataset_paths:
            print(f"No NPZ files found in {args.deduplicate}")
            return

        output_dir = args.deduplicate / "deduplicated"
        output_dir.mkdir(exist_ok=True)

        total_stats = {"original_count": 0, "kept_count": 0, "removed_count": 0}
        ...
        for dataset_path in dataset_paths:
            ...
            logger.info(
                f"Deduplicated {dataset_path.name}: "
