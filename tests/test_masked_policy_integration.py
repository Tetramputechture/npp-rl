...
def test_action_masking_prevents_invalid_actions():
    ...
    batch_size = 4
    obs = {
        "player_frame": torch.randint(0, 255, (batch_size, 64, 64, 1), dtype=torch.uint8),
        "global_view": torch.randint(0, 255, (batch_size, 128, 128, 1), dtype=torch.uint8),
        "game_state": torch.randn(batch_size, 50),
        "action_mask": torch.tensor([[1, 1, 1, 0, 0, 0]] * batch_size, dtype=torch.bool),
    }
    ...
    print(f"   Masked actions: {action_counts[3:].sum().item()} / {num_samples * batch_size} (expected: 0)")
...
def test_evaluate_actions_with_mask_integration():
    ...
    assert torch.all(torch.isfinite(log_probs)), "Valid actions should have finite log probs"
    
    # Test evaluating masked actions - should have -inf log prob
    masked_actions = torch.tensor([3, 4])
    values_masked, log_probs_masked, entropy_masked = policy.evaluate_actions(obs, masked_actions)
    
    assert torch.all(torch.isinf(log_probs_masked)), "Masked actions should have -inf log prob"
...
def test_no_regression_without_mask():
    ...
    print(f"   All actions available: {(action_counts > 0).sum().item()} / 6 actions selected")
