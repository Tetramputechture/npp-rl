...
def create_mock_dataset(tmp_path):
    ...
    stages = ["simplest", "simpler", "simple", "medium", "complex", "exploration", "mine_heavy"]
...
def test_mixing_ratio_sync_initialization():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert main_ratio == 0.40, f"Expected 40% mixing for 40% success, got {main_ratio:.1%}"
        ...
        for i in range(n_envs):
            ...
            if isinstance(env, CurriculumEnv):
                ...
                logger.info(f"Env {i} cached mixing ratio: {cached_ratio:.1% if cached_ratio else 'None'}")
                assert cached_ratio is not None, f"Env {i} should have cached mixing ratio"
                assert cached_ratio == 0.40, f"Env {i} should have 40% mixing ratio, got {cached_ratio:.1%}"
...
def test_mixing_ratio_adapts_with_performance():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        venv_wrapper = CurriculumVecEnvWrapper(venv, curriculum_manager, check_advancement_freq=10)
        ...
        for i in range(4):
            ...
            if isinstance(env, CurriculumEnv):
                cached_ratio = env.curriculum_manager.stage_mixing_ratios.get("simpler")
                assert cached_ratio == 0.25, f"Env {i} should have 25% mixing after init"
...
def test_subprocess_uses_cached_ratio():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        logger.info(f"Subprocess mixing ratio (with cached value): {subprocess_ratio:.1%}")
        ...
        logger.info("✓ Subprocess correctly uses cached ratio instead of calculating from stale data")
...
def main():
    ...
    for test_fn in test_functions:
        try:
            ...
            logger.error(f"Test {test_fn.__name__} failed: {e}", exc_info=True)
            raise
