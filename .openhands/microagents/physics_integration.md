...
**Use these states when implementing movement classification or trajectory calculation.**

## Physics-Accurate Calculations

### Trajectory Calculation Pattern

```python
def calculate_jump_trajectory(src_pos: Tuple[float, float], 
                            tgt_pos: Tuple[float, float],
                            ninja_state: Optional[NinjaState] = None) -> TrajectoryResult:
    """Calculate jump trajectory using actual N++ physics."""
    
    dx = tgt_pos[0] - src_pos[0]
    dy = tgt_pos[1] - src_pos[1]
    
    # Use actual N++ gravity constants
    if dy < JUMP_THRESHOLD_Y:  # Upward movement
        gravity = GRAVITY_JUMP  # Reduced gravity while jumping
        initial_y_velocity = JUMP_FLAT_GROUND_Y
    else:
        gravity = GRAVITY_FALL  # Full gravity when falling
    
    # Calculate time of flight using actual physics
    # y = initial_y + v_y * t + 0.5 * g * t^2
    # Solve quadratic equation for time
    a = 0.5 * gravity
    b = initial_y_velocity
    c = dy
    
    discriminant = b * b - 4 * a * c
    if discriminant < 0:
        return TrajectoryResult(feasible=False, reason="No real solution")
    
    t1 = (-b + math.sqrt(discriminant)) / (2 * a)
    t2 = (-b - math.sqrt(discriminant)) / (2 * a)
    time_of_flight = max(t1, t2) if max(t1, t2) > 0 else min(t1, t2)
    
    # Calculate required horizontal velocity
    if time_of_flight <= 0:
        return TrajectoryResult(feasible=False, reason="Invalid flight time")
    
    required_velocity = dx / time_of_flight
    
    # Check against speed limits
    if abs(required_velocity) > MAX_HOR_SPEED:
        return TrajectoryResult(feasible=False, reason="Exceeds max speed")
    
    return TrajectoryResult(
        feasible=True,
        time_of_flight=time_of_flight,
        required_velocity=required_velocity,
        energy_cost=calculate_energy_cost(required_velocity, time_of_flight)
    )
```

### Movement Classification Using Physics

```python
class MovementClassifier:
    """Classify movements based on N++ physics constraints."""
    
    def classify_movement(self, src_pos: Tuple[float, float], 
                         tgt_pos: Tuple[float, float],
                         ninja_state: Optional[NinjaState] = None) -> Tuple[MovementType, Dict]:
        """Classify movement type using physics constraints."""
        
        dx = tgt_pos[0] - src_pos[0]
        dy = tgt_pos[1] - src_pos[1]
        distance = math.sqrt(dx * dx + dy * dy)
        
        # Use actual ninja radius for collision detection
        if distance <= NINJA_RADIUS * 2:
            return MovementType.STATIONARY, {'distance': distance}
        
        # Vertical movement threshold based on jump physics
        if abs(dy) < VERTICAL_MOVEMENT_THRESHOLD:
            # Horizontal movement - check if walkable
            required_speed = distance / self._estimate_walk_time(distance)
            
            if required_speed <= MAX_HOR_SPEED:
                return MovementType.WALK, {
                    'time_estimate': distance / MAX_HOR_SPEED,
                    'energy_cost': ENERGY_COST_BASE * distance
                }
        
        # Vertical movement - requires jumping
        if dy < -NINJA_RADIUS:  # Upward movement
            trajectory = self._calculate_jump_trajectory(src_pos, tgt_pos)
            
            if trajectory.feasible:
                return MovementType.JUMP, {
                    'time_estimate': trajectory.time_of_flight,
                    'energy_cost': trajectory.energy_cost,
                    'success_probability': self._calculate_success_probability(trajectory)
                }
        
        # Check for wall jumping scenarios
        if ninja_state and ninja_state.wall_contact:
            return self._classify_wall_movement(src_pos, tgt_pos, ninja_state)
        
        return MovementType.COMPLEX, {'requires_pathfinding': True}
```
